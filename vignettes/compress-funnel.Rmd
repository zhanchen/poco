...
It requires a working CmdStan install. The Stan-fitting chunks below are
gated with `instantiate::stan_cmdstan_exists()`, so the vignette still
...
```{r fit_funnel, eval = instantiate::stan_cmdstan_exists()}
library(cmdstanr)
...
```{r compress_csv, eval = instantiate::stan_cmdstan_exists()}
...
```{r regenerate, eval = instantiate::stan_cmdstan_exists()}
...
```{r plots, message = FALSE, warning = FALSE, eval = instantiate::stan_cmdstan_exists()}
library(ggplot2)
