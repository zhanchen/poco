...
two-component example. See `vignette("compress-brms")`,
`vignette("compress-funnel")`, and `vignette("compress-sccomp")` for
end-to-end workflows on real `brms`, `cmdstanr`, and `sccomp` fits, and
`vignette("methods-benchmark")` for a side-by-side comparison of the
...
- `vignette("compress-brms", package = "poco")` - full
  `brms::brmsfit` round-trip, including reconstruction.
